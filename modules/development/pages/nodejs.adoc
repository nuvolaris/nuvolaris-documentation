...
Notice that the `+main+` function returns a Promise, which indicates
that the activation hasn’t completed yet, but is expected to in the
future.
...
Comparing the `+start+` and `+end+` time stamps in the activation
record, you can see that this activation took slightly over two seconds
to complete.
...
Using the `+--result+` flag means that the value returned from the
action is shown as output on the command-line:
...
packaged as a zip file MUST contain a valid `+package.json+` with a
`+main+` field used to denote the
...
[arabic]
. Run `+npm install+` inside a Docker container from the platform
images.
...
_Make sure you export the function using the `+const main = ...+`
pattern. Using `+export {myAction as main}+` does not work due to
tree-shaking. See this
