...
Pass to `nuv web upload` the path to folder where the index.html is stored in (the `web` folder if you followed the tutorial) and visit again `<apihost>`. 
...
/nuvolaris/hello    private     <-- a default package created during development
----
