...
Once you registrered a `<dns-name>` (for example `nuvolaris.example.com`) or a wildcard DNS name (for example `*.example.com`) you can communicate to the installer what is this DNS name, as it is not able to detect it automatically.

[NOTE]
====
If you use a wildcard DNS name, you have to choose anyway a DNS name in the DNS to be used as the api endpoint.
====
