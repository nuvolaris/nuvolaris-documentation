...
Nuvolaris will invoke this action as soon as possible, so we can go on.
...
function save(doc) {
  var ow = openwhisk({api_key: <whisk auth>})
...
To construct the `ow` object you need to pass the API key, which you can get with:
...
Nuv embeds the wsk cli tool, so you have access to it via `-wsk`.
