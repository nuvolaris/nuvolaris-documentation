...
OpenWhisk supports link:actions.md#languages-and-runtimes[several
...
integration with the platform. The OpenWhisk platform is extensible and
...
OpenWhisk platform, and officially recognized by the Apache OpenWhisk
...
property called ``kind''. When using the `wsk` CLI, this is specified as
...
The initialization route is called exactly once by the OpenWhisk
...
*Time limit:* Every action in OpenWhisk has a defined time limit (e.g.,
...
in the OpenWhisk platform itself. Note that even if the context is
...
completed initialization. The OpenWhisk platform will invoke the
...
otherwise the OpenWhisk platform will treat the activation as a failure
...
function activation.
* `namespace` is the OpenWhisk namespace for the action (e.g.,
`whisk.system`).
...
`__OW_API_HOST` whose value is the OpenWhisk API host. This value is
...
respond with a JSON object, the OpenWhisk platform will treat the
...
Since the OpenWhisk platform is language and runtime agnostic, it is
...
some cases to modify the `wsk` CLI or other OpenWhisk clients. In which
case, appropriate tests should be added as necessary. The OpenWhisk
...
addition to OpenWhisk. Currently only an interface for single entrypoint
...
standard OpenWhisk runtime environments; instead both the initialization
